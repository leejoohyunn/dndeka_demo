// functions/index.js - v6 + v1 명시적 사용
...
// 서비스들을 안전하게 로드
let classifyAppetite, missionHandler;

try {
  const appetiteClassifierModule = require('./services/appetiteClassifier');
  classifyAppetite = appetiteClassifierModule.classifyAppetite;
} catch (err) {
  console.error('appetiteClassifier load error:', err);
  classifyAppetite = () => Promise.resolve('physiological'); // 기본값
}

try {
  const missionHandlerModule = require('./handlers/missionHandler');
  missionHandler = missionHandlerModule.missionHandler;
} catch (err) {
...
// 챗봇 메시지 처리 함수 - v1 방식
const processMessage = functions
...
**${missionResult.missions[0]?.title || '기본 미션'}**
${missionResult.missions[0]?.description || '설명이 없습니다.'}
...
수행 시간: ${mission.timeConstraint}`;
            ...
            botResponse = `아직 설정된 미션이 없습니다. 새로운 미션을 생성하겠습니다.`;
          }
        } else {
          // 일반적인 응답
...
// 처음 로그인한 사용자에게 자동으로 첫 질문 보내기 - v1 방식
const initializeNewUser = functions
  .region('asia-northeast3')
  .https.onCall(async (data, context) => {
    if (!context.auth) {
      throw new functions.https.HttpsError('unauthenticated', '로그인이 필요합니다.');
